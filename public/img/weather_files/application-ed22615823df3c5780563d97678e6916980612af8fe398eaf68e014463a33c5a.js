          ...
          $.ajax({
            ...
            .success(function(s3Response) {
              ...
              var url = !!onboardingCollectionId
                ...
                : 'api/v4/items/' + item.slug + '/completed';
              $.ajax({
                method: 'PUT',
                url: host + url,
                dataType: 'json',
...
(function ($) {
  function updateEmbed() {
    ...
    if (typeof gon === 'undefined' || !gon.item || !gon.item.shareURL) return;

    $.ajax({
      dataType: "json",
      url: gon.oembed_service,
      data: {
        url: gon.item.shareURL,
        format: "json"
      },
      success: function (result) {
        $("#embed-html").val(result.html);
...
(function() {
  $(document).on("ready", function() {
    let loadVideoJS = function(videoElement, sourceElement) {
      if (videoElement && typeof videojs === "function") {
    ...
    let loadHlsJs = function (videoElement, sourceElement) {
      if (typeof Hls === "function" && Hls.isSupported()) {
        var player = new Hls();
        player.attachMedia(videoElement);

        player.on(Hls.Events.MEDIA_ATTACHED, function () {
          player.loadSource(sourceElement.src);
        });
          ...
          if (data.fatal) {
            switch (data.type) {
              case Hls.ErrorTypes.NETWORK_ERROR:
                console.log("hlsjs fatal network error encountered, trying to recover");
                player.startLoad();
                break;
              case Hls.ErrorTypes.MEDIA_ERROR:
                console.log("hlsjs fatal media error encountered, trying to recover");
    ...
    // skip if load from Vue
    if ($("#viewer-vue").length === 0) {
      let videoElement = document.getElementById("video-item");
      if (videoElement) {
        let sourceElement = videoElement.querySelector("source");
        let videoType = sourceElement.getAttribute("type");

        if (videoType === "video/mp4") {
          loadVideoJS(videoElement, sourceElement);
        }
        else if (videoType === "application/x-mpegURL") {
          loadHlsJs(videoElement, sourceElement);
        }
      }
    }
  });
...
(function($) {
  ...
  $('#sidebar-layout-wrapper .sidebar-toggle').click(function(e) {
    $('#sidebar-yield').toggleClass('d-none');
  ...
  function ajaxRequestForBulkOperation(requestObj) {
    if ($(".drops-check:checkbox:checked").length <= 0) {
      alert('Please select atleast one item');
      return;
    }
    var selectedIds = [];
    $(".drops-check:checkbox:checked").each(function(e){
    ...
    $.ajax({
      ...
      success: function(data) {
        let metadata_message = (data.metadata && data.metadata.message) ? data.metadata.message : null;
        window.localStorage.setItem('flash', metadata_message);
        window.location.reload();
      },
  ...
  $('#bulk_delete_items').off('click').on('click', function(e){
    let response = confirm("Are you sure you want to archive?");
    if (response) {
      ajaxRequestForBulkOperation({url: '/api/v4/items/bulk_destroy', method: 'delete'})
    }
  });

  $('#bulk_favorite_items').off('click').on('click', function(e){
    ajaxRequestForBulkOperation({url: '/api/v4/items/bulk_favorite', method: 'put'});
  });
...
(function() {
  $(document).on("ready", function() {
    let keepTabState = function() {
      ...
      if (hash) {
        $('.nav a.nav-link[href="' + hash + '"]').tab('show');
      } else {
        $('.nav a.nav-link:first').tab('show');
      }
...
$(document).ready(function() {
  ...
  let paramMap = {
    show_share: "[id^=security-settings-modal]", // For some reason this has a slug appended.
    show_embed: "#embedModal",
    show_collection: "#collectionModal"
  };

  for (let [p, id] of Object.entries(paramMap)) {
    if (params.get(p)) {
      // I have no idea why, but this won't run without the setTimeout.
      setTimeout(function() {
        $(id).modal("show");
      }, 0);
    }
...
(function($) {
  ...
  let jwtProtection = function(xhr) {
      let token = jwt.get();
      if (token) xhr.setRequestHeader('Authorization', 'Bearer: ' +  token);
  }
  $.ajaxPrefilter(function(options, originalOptions, xhr){ if ( !options.crossDomain ) { jwtProtection(xhr); }});


  $('body').on('ajax:beforeSend', 'form, a.remote-link', function(event) {
    let detail = event.originalEvent.detail;
    let xhr = detail[0];
    xhr.setRequestHeader('Authorization', jwt.get());
  });
  ...
  $('body').on('ajax:success', 'form, a.remote-link', function(event) {
    ...
    let message = body.message || metadata_message;
    if (redirect_to) {
...
$(document).ready(() => {
  ...
  $("body").on("click", "[event]", function() {
    ...
    dataContent = jQuery.isEmptyObject($(this).data()) ? $(this).attr("data") : $(this).data();
    $.ajaxSetup( {
      beforeSend: function ( xhr ) {
        xhr.setRequestHeader( 'X-CSRF-Token', token );
      }
    });
    $.post( "/tracking", { event: eventName, properties: dataContent } );
  });
});
(function($) {
  let flash;
  ...
  if (flash) {
    if (typeof flash == 'string') {
      ...
      $alertFlash = $(`.alert-${flash['type']}`);
      $alertFlash.show().text(flash['message']);
      setTimeout(function() {
        $alertFlash.fadeOut('slow');
      }, 2000);
    }
  }
...
// this javascript caused reloads when it shouldn't have.
//  require pusher.subscription
;
